...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = chars match {
    case Nil => Nil
    case x :: xs =>
		  val (first, rest) = xs.partition(ch => x == ch)
      (x, 1+first.length) :: times(rest)
  }
   ...
   */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith((a,b) => a._2 < b._2).map( x => Leaf(x._1, x._2))
  ...
  def printTree(tree: CodeTree) = tree match {
    case Leaf(ch, _) => println("Leaf("+ch+")")
    case Fork(_,_,chars,_) => println("Fork("+chars+")")
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
	  ...
	  def iter(subtree: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] = bits match {
		  case List() => subtree match {
        case Leaf(ch, _) => acc :+ ch
        case _ => acc
      }
      case x :: xs => subtree match {
        case Leaf(ch, _) =>
          iter(tree, x :: xs, acc :+ ch)
        case Fork(left, right, chars, _) =>
          if (x == 0) iter(left, xs, acc)
          else iter(right, xs, acc)
		  }
	  }
  ...
  val frenchCode: CodeTree = Fork(
      ...
      Fork(
        Leaf('n',108812),
        Leaf('t',111103),List('n','t'),219915
      ),List('r','c','v','g','b','n','t'),422503
    ),
    Fork(Leaf('e',225947),Fork(Leaf('i',115465),Leaf('a',117110),List('i','a'),232575),List('e','i','a'),458522),List('r','c','v','g','b','n','t','e','i','a'),881025),List('s','d','x','j','f','z','k','w','y','h','q','o','l','m','p','u','r','c','v','g','b','n','t','e','i','a'),1486387)
  ...
  //val secret: List[Bit] = List(0,0,1,1,1,1,0,1)
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    def iter(subtree: CodeTree, acc: CodeTable, bits: List[Bit]): CodeTable = subtree match {
      case Leaf(ch, _) => (ch, bits.reverse) :: acc
      case Fork(left, right, _, _) => iter(left, acc, 0 :: bits) ::: iter(right, acc, 1 :: bits)
    }
